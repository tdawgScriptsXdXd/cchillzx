...
-- Soft glowing orb behind the panel that colour-shifts
-- ══════════════════════════════════════════════════════════
...
task.spawn(function()
    local seq = {P.vi, P.cy, P.pk, P.gr, P.vi}
    local i = 1
    while bloom and bloom.Parent do
        tw(bloom, {BackgroundColor3 = seq[(i % #seq) + 1]}, TweenInfo.new(2.8, Enum.EasingStyle.Sine))
        i += 1; task.wait(2.8)
    end
end)
...
-- Tiny coloured orbs float upward continuously
-- ══════════════════════════════════════════════════════════
...
for _ = 1, 16 do
    task.spawn(function()
        ...
        while gui and gui.Parent do
            ...
            local driftX = math.random(18, 300) + math.random(-20, 20)
            TweenService:Create(p, TweenInfo.new(dur, Enum.EasingStyle.Linear), {
                Position = UDim2.new(0, driftX, -0.05, 0),
...
-- UIStroke smoothly sweeps through the full palette
-- ══════════════════════════════════════════════════════════
local mainStr = Instance.new("UIStroke", frame)
mainStr.Thickness    = 1.5
mainStr.Color        = P.vi
mainStr.Transparency = 0.35
task.spawn(function()
    local seq = {P.vi, P.cy, P.pk, P.gr, P.am, P.cy, P.vi}
    local i = 1
    while frame and frame.Parent do
        tw(mainStr, {Color = seq[(i % #seq) + 1], Transparency = 0.28},
            TweenInfo.new(1.7, Enum.EasingStyle.Sine))
        i += 1; task.wait(1.7)
    end
end)
...
-- Frame springs in from the left with bounce easing
-- ══════════════════════════════════════════════════════════
...
topFix.BackgroundColor3 = P.surface
...
topSep.BackgroundColor3      = P.vi
topSep.BackgroundTransparency = 0.65
...
-- Two quick pulses then a rest — like a real heartbeat
-- ══════════════════════════════════════════════════════════
local dot = Instance.new("Frame")
dot.Size             = UDim2.new(0, 8, 0, 8)
dot.Position         = UDim2.new(0, 13, 0.5, -4)
dot.BackgroundColor3 = P.vi
...
task.spawn(function()
    ...
    while dot and dot.Parent do
        beat(UDim2.new(0,12,0,12), UDim2.new(0,11,0.5,-6),
             UDim2.new(0, 8, 0, 8), UDim2.new(0,13,0.5,-4), 0.18)
        beat(UDim2.new(0,11,0,11), UDim2.new(0,11.5,0.5,-5.5),
             UDim2.new(0, 8, 0, 8), UDim2.new(0,13,0.5,-4), 0.88)
    end
end)
...
body.ScrollBarImageColor3  = P.vi
body.CanvasSize            = UDim2.new(0, 0, 0, 0)
body.AutomaticCanvasSize   = Enum.AutomaticSize.Y
body.ZIndex                = 4
body.Parent                = frame
local bodyL = Instance.new("UIListLayout")
bodyL.SortOrder  = Enum.SortOrder.LayoutOrder
bodyL.Padding    = UDim.new(0, 6)
bodyL.Parent     = body
...
local function newCard(h, lo, ac)
    ...
    strip.Size             = UDim2.new(0, 3, 0.56, 0)
    strip.Position         = UDim2.new(0, 0, 0.22, 0)
    strip.BackgroundColor3 = ac
...
local function lbl(p, t, x, y, w, h, sz, col, font, align)
    local l = Instance.new("TextLabel")
    l.Size               = UDim2.new(w, 0, 0, h)
    l.Position           = UDim2.new(0, x, 0, y)
    l.BackgroundTransparency = 1
    l.Text               = t
    l.TextColor3         = col or P.tx
    l.TextSize           = sz or 10
...
tickFrame.LayoutOrder        = 0
tickFrame.ClipsDescendants   = true
tickFrame.ZIndex             = 5
tickFrame.Parent             = body
Instance.new("UICorner", tickFrame).CornerRadius = UDim.new(0, 7)
local tickStr = Instance.new("UIStroke", tickFrame)
tickStr.Color = P.vi; tickStr.Transparency = 0.8
local tickLbl = Instance.new("TextLabel")
tickLbl.Size             = UDim2.new(5, 0, 1, 0)
tickLbl.BackgroundTransparency = 1
tickLbl.Text             = ""
tickLbl.TextColor3       = P.dim
tickLbl.TextSize         = 7.5
tickLbl.Font             = Enum.Font.Gotham
tickLbl.TextXAlignment   = Enum.TextXAlignment.Left
tickLbl.ZIndex           = 6
tickLbl.Parent           = tickFrame
local tOff = 0
task.spawn(function()
    local last = os.clock()
...
ava.Position         = UDim2.new(0, 10, 0.5, -16)
ava.BackgroundColor3 = Color3.fromRGB(18, 14, 40)
...
avaL.TextColor3 = P.vi; avaL.TextSize = 15; avaL.Font = Enum.Font.GothamBlack
avaL.ZIndex = 8; avaL.Parent = ava
lbl(pCard, LocalPlayer.Name,           50, 7,  0.55, 15, 10.5, P.tx,   Enum.Font.GothamBlack)
lbl(pCard, "ASSASSIN  ·  SERVER LOCKED", 50, 22, 0.55, 12, 7.5, P.dim, Enum.Font.GothamBold)

local rBadge = Instance.new("Frame")
rBadge.Size             = UDim2.new(0, 42, 0, 34)
rBadge.Position         = UDim2.new(1, -48, 0.5, -17)
rBadge.BackgroundColor3 = P.ele
...
rNum.TextColor3 = P.vi; rNum.TextSize = 15; rNum.Font = Enum.Font.GothamBlack
...
local function updateUI()
    if not enabled then
        ...
        for _, s in ipairs(segs) do
            tw(s, {BackgroundColor3 = P.ghost, BackgroundTransparency = 0.88},
...
setBtn.MouseButton1Click:Connect(function()
    ...
    if v and v > 0 and v <= 120 then
        ...
        feedLbl.Text = "✗ Enter 1–120"; feedLbl.TextColor3 = P.rd
    end
    task.delay(2.5, function() feedLbl.Text = "" end)
end)
...
toggleBtn.MouseButton1Click:Connect(function()
    enabled = not enabled
    if enabled then
        toggleBtn.Text = "⟳  AUTO: ON"; toggleBtn.TextColor3 = P.vi; startTimer()
    else
        toggleBtn.Text = "⟳  AUTO: OFF"; toggleBtn.TextColor3 = P.rd; updateUI()
    end
end)
